...
- **Mobile galleries are automatic.** If the master has per-length `<L>-<MODEL>-MOBILE/` folders (portrait **4:5 @ 1080×1350**, *gallery images only* — HERO/GALLERY-THUMB stay shared from the desktop set), `apply_model_photos.sh` auto-runs `apply_mobile_gallery.sh` at the end: it copies those portraits to `assets/photos/<slug>/mobile/` (same dest names as desktop) and writes `<script>window.WB_MOBILE=…</script>`, so on phones (≤700px) the shared lightbox swaps to the portrait shots — same gallery, taller images. Orphans (a mobile shot with no same-name desktop photo) are auto-skipped so mobile always mirrors desktop. No `*-MOBILE` folders → silent no-op. Run standalone for a mobile-only pass: `_build/apply_mobile_gallery.sh <slug> "$SRC"` (prefix `PHOTOSLUG=` if page≠photo dir).
